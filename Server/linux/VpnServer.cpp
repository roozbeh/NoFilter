...
killall VpnServer
for i in {0..20}; do 
    nohup ./VpnServer tun$i 8000 test -m 1400 -a 10.0.0.$((i*2 + 2)) 32 -d 8.8.8.8 -r 0.0.0.0 0 &
done
...
// ./VpnServer tun0 8000 test -m 1400 -a 10.0.0.2 32 -d 8.8.8.8 -r 0.0.0.0 0
//
...
static void log_message(const char *log) {
    int pid = getpid();
    printf("[%d] %s\n", pid, log);
}

static void log_message2(const char *log, int d) {
    int pid = getpid();
    char log_mixed[1024];
    sprintf(log_mixed, log, d);
    printf("[%d] %s\n", pid, log_mixed);
}
...
static int get_tunnel(char *port, char *secret)
{
    ...
    for(;;)
    {
        ...
        if(pid > 0)
        {
            close(tunnel);
            counter++;
            printf("parent processes going to continue to listen, counter: %d\n", counter);
            continue;
        }
        if (pid == 0) {
            break;
...
int main(int argc, char **argv)
{
    ...
    log_message2("interface: %d\n", interface);

    // Wait for a tunnel.
    int tunnel;
    while ((tunnel = get_tunnel(argv[2], argv[3])) != -1) {
        log_message2("Here comes a new tunnel: %d\n", tunnel);
        ...
        while (true) {
            ...
            if (length > 0) {
                ...
                // printf("read %d bytes from interface, sending to tunnel\n", length);
	    ...
	    if (length > 0) 
            // printf("recv %d bytes from tunnel\n", length);
            ...
            if (length > 0) {
                ...
                if (packet[0] != 0) {
                    ...
                    // printf("ignoring control packaet: %d bytes\n", length);
		        }
            ...
            if (idle) {
                ...
                if (timer < -16000) {
                    ...
                    packet[0] = 0;
                    for (int i = 0; i < 3; ++i) {
			            log_message("seding no signal message to tunnel\n");
                ...
                if (timer > 20000) {
		            log_message("sending for long time, not recv!\n");
                    break;
                }
            }
        }
        log_message("The tunnel is broken\n");
        close(tunnel);
    }
